...
function buildPaidFetch() {
  ...
  if (!key) throw new Error('WALLET_PRIVATE_KEY not set — run: export WALLET_PRIVATE_KEY=0x...')
  const account = privateKeyToAccount(key as `0x${string}`)
  const wallet = createWalletClient({ account, chain: base, transport: http() })
  return wrapFetchWithPayment(fetch, wallet as any)
}

export async function runSkill(skill: SkillDef, args: Record<string, unknown>): Promise<unknown> {
  const paidFetch = buildPaidFetch()
  const body = skill.buildBody(args)
  ...
  if (!res.ok) {
    ...
    throw new Error(`[blueagent/${skill.endpoint}] ${res.status}: ${text}`)
  }
